...
export const MinecraftDashboard = (props: Props) => {
    ...
    const [workloads, setWorkloads] = React.useState(undefined);
    ...
    const issueCommand = () => {
        if (ready && window.socket) {
            window.socket.send(
                JSON.stringify({ type: MessageType.IssueCommand, data: command.trim() })
            );
            setCommand("");
        }
    };
    ...
    const connect = () => {
        ...
        window.socket.addEventListener("message", (msg) => {
            ...
            switch (message.type) {
                ...
                case MessageType.Connect:
                    {
                        ...
                        if (message.data.success) {
                            ...
                            props.openSnackbar(
                                ...
                                `Connection established! Your connection is being monitored`,
                                4000
                            );
                        } else {
                            closing = true;
                            props.openSnackbar(
                                ...
                                `Failed to establish connection: ${message.data.message}`,
                                4000
                            );
                        }
                    }
                ...
                case MessageType.StartMonitor:
                    {
                        // a response here means we failed to start monitoring
                        setLogs("");
                        receivedLogs = [];
                ...
                case MessageType.MonitorReceive:
                    {
                        ...
                        if (console && !disableJump) console.scrollTop = console.scrollHeight;
                    }
                    break;

                case MessageType.IssueCommand:
                    {
                        // a response here means the command failed to send
                        props.openSnackbar(SnackbarStatus.Error, `${message.data}`, 4000);
                    }
                    break;
                ...
                case MessageType.Restart:
                    {
                        // a response here means the restart failed
                        setRestarting(false);
                        props.openSnackbar(SnackbarStatus.Error, `${message.data}`, 4000);
                    }
                    break;
            ...
            if (window.socket)
                window.socket.send(
                    JSON.stringify({
                        ...
                        data: { key: apiKey, apiVersion: API_VERSION }
                    })
                );
        };
    ...
    const updateStoreApiKey = (checked: boolean) => {
        ...
        if (checked) localStorage.setItem("apikey", apiKey);
        else localStorage.setItem("apikey", "");
    };

    const updateApiKey = (value: string) => {
        setApiKey(value);
        if (storeApiKey) localStorage.setItem("apikey", value);
    };
    ...
    const consoleScroll = (e: React.UIEvent<HTMLDivElement>) => {
        ...
        if (div.scrollTop + div.offsetHeight - div.scrollHeight >= -10) disableJump = false;
        else disableJump = true;
    };
    ...
    React.useEffect(() => {
        ...
        if (savedStoreKey) setStoreApiKey(savedStoreKey == "true");
    ...
    return (
        <div>
                ...
                >
                    ...
                    <FormControlLabel
                        control={
                            <Checkbox
                                checked={storeApiKey}
                                onChange={(e) => updateStoreApiKey(e.target.checked)}
                            />
                        }
                        label="Remember API key"
                        style={{ marginBottom: "0.25rem", color: theme.PALETTE_WHITE }}
                    />
                    <Button variant="outlined" disabled={connecting} onClick={connect}>
                        Connect
                    </Button>
                </div>
            )}
            {ready &&
                (isAdmin ? (
                    <AdminPage openSnackbar={props.openSnackbar} />
                ) : (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                        {support && (
                            <Typography variant="h4" style={{ color: theme.PALETTE_RED }}>
                                SUPPORT CONNECTION
                            </Typography>
                        )}
                        ...
                        >
                            <Button
                                variant="outlined"
                                disabled={restarting}
                                onClick={() => startMonitor(false)}
                            >
                                Reconnect to log
                            </Button>
                            <Button
                                variant="outlined"
                                disabled={logs == "" || restarting}
                                onClick={stopMonitor}
                            >
                                Stop loading log
                            </Button>
                            ...
                            <Button variant="outlined" disabled={restarting} onClick={stopServer}>
                                Stop server
                            </Button>
                            <Button variant="outlined" disabled={restarting} onClick={startServer}>
                                Start server
                            </Button>
                            <Button
                                variant="contained"
                        ...
                        >
                            ...
                            <Button
                                variant="outlined"
                                disabled={command.trim() == "" || restarting}
